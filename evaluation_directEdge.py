...
def mask2target(mask):
    ...
    return outlineMask*2.0-1., importance
...
class EdgeImportanceLoss(_Loss):
    ...
    def forward(self, x, target, importance):
        ...
        error = abs(x-target)
        importanceError = (error*importance)
        missedPositiveError = 100*(error*hasToBePos).sum()/(hasToBePos.sum()+0.000001)
        missedNegativeError = (importanceError*hasToBeNeg).sum()/((hasToBeNeg*importance).sum()+0.000001)
...
class DepthJointRCNN(DepthRCNN):
    ...
    def forward(self, batched_inputs):
        ...
        loss1 = sum(losses.values())
        loss2 = edgeSegmentLoss["missedPositiveError"]+edgeSegmentLoss["missedNegativeError"]
        losses["missedPositiveError"] = edgeSegmentLoss["missedPositiveError"]
        losses["missedNegativeError"] = edgeSegmentLoss["missedNegativeError"]
...
class JointDepthEvaluator(COCOEvaluator):
    ...
    def evaluate(self):
        ...
        for prediction in self._predictions:
            ...
            for i, annotation in enumerate(prediction["instances"]):
                maskThreshold = 0.6
                segmentation = annotation['segmentation']
                mask = self._decode_binImage(segmentation, prediction["height"], prediction["width"])
                if prediction["scores"][i] < maskThreshold:
                    continue
            ...
            for key in range(len(keys)):
                ...
                for target_nr in range(len(targetAreas)):
                    ...
                    if relevantResults.sum() == 0:
                        ...
                        target_fragmentation.append(torch.tensor([0.]))
    ...
    def process(self, inputs, outputs):
        ...
        save = {"image_id": inputs[0]["image_id"],
                ...
                "scores":outputs["MaskRCNN"][0]["instances"].scores.to(self._cpu_device),
                "edges":rles}
