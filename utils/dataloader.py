...
from utils.pascal_ctxt import pascalVOCContextLoader, customDatasetContextLoader
...
class CobDataLoader(Dataset):
    def __init__(self,
        ...
        self.dl = customDatasetContextLoader(root_imgs, root_segs, split=split)
    ...
    def prepare_all(self):
        if (not os.path.exists(self.or_cntr_path)):
            ...
            dl = customDatasetContextLoader(self.root_imgs, self.root_segs)
            dl.split = 'train'
            for ii in tqdm(range(len(dl))):
                    s = dl[ii]
                    or_cntr = interpolate_to_polygon(s["labels"][:,:,0] + 255*s["labels"][:,:,1] + 255*255*s["labels"][:,:,2]).astype(
                        np.uint8)
